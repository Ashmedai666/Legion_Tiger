...
const Hero = () => {
  ...
  return (
    <div className="relative h-screen flex items-center justify-center overflow-hidden bg-stalker-900">
      ...
      {/* Scroll Indicator */}
      <div className="absolute bottom-10 left-1/2 transform -translate-x-1/2 flex flex-col items-center gap-2 opacity-50 animate-pulse">
...
const CategoryCard = ({ title, subtitle, image, link, colSpan }: { title: string, subtitle: string, image: string, link: string, colSpan: string }) => (
  <Link to={link} className={`group relative h-[500px] overflow-hidden block border border-white/5 ${colSpan}`}>
    <img 
      src={image} 
      alt={title} 
